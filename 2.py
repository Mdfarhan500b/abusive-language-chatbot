...
offensive_words = ["fuck","kill","fool", "asshole", "bitch","damn", "bastard", "whore", "slut", "pussy","dickhead"]
...
def get_response():
    try:
        ...
        # First, check for predefined offensive words
        if any(offensive_word in user_message.lower() for offensive_word in offensive_words):
            return jsonify({'response': 'This message contains offensive language.'})
        ...
        if prediction == 1:  # Assuming 1 represents offensive language
            ...
            return jsonify({'response': 'This message is not offensive.'})
